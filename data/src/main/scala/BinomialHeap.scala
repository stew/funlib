  ...
  def uncons(implicit order: Order[A]): Maybe[(A,BinomialHeap[A])] = {
    ...
    minTree flatMap { mt ⇒ 
      ...
      for {
        ...
        val x = _insertTree(t, newTrees)
      }
      removedOpt.flatMap { removed ⇒
...
object BinomialHeap extends BinomialHeapInstances {
...
trait BinomialHeapInstances {

  implicit def binHeapOrder[A](implicit O: Order[A]): Order[BinomialHeap[A]] = new Order[BinomialHeap[A]] {
    @tailrec
    def apply(left: BinomialHeap[A], right: BinomialHeap[A]) = 
    (left.uncons,right.uncons) match {
      case (NotThere(), NotThere()) => EQ
      case (x @ There(_), NotThere()) => LT
      case (NotThere(), x @ There(_)) => GT
      case (There((lh,lt)), There((rh,rt))) => O(lh,rh) match {
        case EQ => this.apply(left,right)
        case x => x
      }
    }
  }
