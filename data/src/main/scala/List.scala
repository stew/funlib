...
object List {
  ...
  implicit val listInstances: Monad[List]/* with Cobind[List] with Foldable[List] with Traverse[List]*/ = new Monad/*Plus*/[List] /*with Cobind[List] with Foldable[List]with Traverse[List]*/ {
    override def point[A](a: ⇒ A) = Cons(a, End())
    override def flatMap[A,B](fa: List[A])(f: A⇒List[B]) = fa.foldr[List[B]]({(i,r) ⇒ f(i) ++ r}, End())
...
//    }
    ...
    def traverseImpl[F[_], A, B](l: List[A])(f: A => F[B])(implicit F: Applicative[F]): F[List[B]] = {
      F.map(l.foldl[F[NastyListAppend[B]]]((fbs, a) => F.map2(fbs, f(a))(_ += _), F.point(new NastyListAppend[B])))(_.run)
    }
  }
